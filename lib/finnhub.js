...
export const getStockProfile = async (stockSymbol) => {
	...
	return await Promise.all([ fetcherProfile, fetcherQuote ])
		.then(async ([ fetchedProfile, fetchedQuote ]) => {
		...
		.catch((e) => {
			...
			console.log(`Error, an error occurred in fetching ${stockSymbol}'s profile data.`);
			console.log('Error: ', e);
			return DEFAULT_DATA;
		});
};
